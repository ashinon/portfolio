  ...
  settings = jQuery.extend(
    {
      ...
      autoPlayDoNotRepeat: true, // 1周だけで止まるフラグ
      autoPlayInterval: {},
      autoPlayPause: 6000, // value: integer (1000 = 1 seg), default to 2000 (2segs)
    },
    options
  ...
  $(function() {
    ...
    if ($(settings.datesDiv).length > 0 && $(settings.issuesDiv).length > 0) {
      ...
      if (settings.orientation == 'horizontal') {
        ...
        // }
        // if(navigator.userAgent.match(/(iPhone|iPad|iPod|Android)/)) {
        //     console.log('iPhone|iPad|iPod|Android');
        //     $(settings.datesDiv).css('marginLeft',0);
        //     var defaultPositionDates = parseInt($(settings.datesDiv).css('marginLeft','0'));
      ...
      $(settings.nextButton).bind('click', function(event) {
        ...
        if (settings.orientation == 'horizontal') {
          ...
          console.log('currentPositionIssues: ' + currentPositionIssues);
          ...
          console.log('currentPositionDates: ' + currentPositionDates);
          var currentIssueDate = currentPositionDates - widthDate;
          console.log('currentIssueDate: ' + currentIssueDate);
          if (currentPositionIssues <= -(widthIssue * howManyIssues - widthIssue)) {
            $(settings.issuesDiv).stop();
            // $(settings.datesDiv+' li:last-child a').click();
            $(settings.datesDiv + ' li:first-child a').trigger('click'); //改変
          } else {
      ...
      if (settings.autoPlay == 'true') {
        settings.autoPlayInterval = setInterval('autoPlay()', settings.autoPlayPause);
      }
    }
  });
};
...
function autoPlay() {
  ...
  if (settings.autoPlayDirection == 'forward') {
    if (currentDate.parent().is('li:last-child')) {
      ...
      if (!settings.autoPlayDoNotRepeat || (settings.autoPlayDoNotRepeat && !alreadyPlayedOnce)) {
        currentDate
          .parent()
          .next()
          .find('a')
